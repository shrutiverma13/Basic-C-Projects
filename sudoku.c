...
int main()
{
  int grid[N][N];
printf("\n SUDOKU SOLVER \n");
printf("Enter your sudoku values row wise with spaces: \n");
printf("Enter the next row by clicking enter after giving 9 inputs for better understanding \n");
printf("Enter 0 for a empty box \n");
printf("\nInput vlaues: \n");
    for(int i=0;i<N;i++){
    for(int j=0;j<N;j++)
    scanf("%d", &grid[i][j]);
